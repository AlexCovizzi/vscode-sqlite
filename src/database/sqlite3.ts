...
import { ParsableStream } from './ParsableStream';
...
export class SQLite {
    ...
    static query(cmdSqlite: string, dbPath: string, query: string, callback: (resultSet?: ResultSet, error?: Error) => void) {
        ...
        let error: Error;
        let parsableStream = new ParsableStream(new ResultSetParser());
        ...
        proc.stdout.pipe(parsableStream).once('done', (data: ResultSet) => {
